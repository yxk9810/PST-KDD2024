...
def find_bib_context(xml, dist=200):
    ...
    for item_str in bibr_strs_to_bid_id:
        ...
        # if len(cur_bib_context_pos_start)==0:
...
def len_is_ok(text,dist):
    text = " ".join(text)
    text = re.sub(r'<.*?>',"",text)
    text = [x for x in text.strip().split(" ") if not ("<" in x or "=" in x or ">" in x)]
    if len(text)>450 or dist==400:
        return True
    else:
        return False


def find_bib_context_v1(xml):
    d = []
    bs = BeautifulSoup(xml, "xml")
    bib_to_context = dd(list)
    bibr_strs_to_bid_id = {}
    ...
    for item_str in bibr_strs_to_bid_id:
        ...
        for dist in [150,200,250,300,350,400]:
            text_ls = []
            for pos in cur_bib_context_pos_start:
                text_ls.append(xml[pos - dist: pos + dist].replace("\n", " ").replace("\r", " ").strip())
            
            if len_is_ok(text_ls,dist):
                break
                
        bib_to_context[bib_id] = text_ls
    return bib_to_context